...
namespace CopyrightRegistrationDemo.Controllers
{
    ...
    public class VerifierController : Controller
    {
        ...
        private ModernAuthService _modernAuthService;
        private IConfiguration _configuration;
        private VCService _vcService;
        
        public VerifierController(IOptions<AppSettingsModel> appSettings, IMemoryCache memoryCache, ILogger<VerifierController> log, IHttpClientFactory httpClientFactory, ModernAuthService modernAuthService, IConfiguration configuration, VCService vcService)
        {
            ...
            _modernAuthService = modernAuthService;
        ...
        {
            //Get token
            authtoken token = await _modernAuthService.GetAccessTokenByCC();
            ...
            myPayload.authority = _configuration["AppSettings:IssuerAuthority"];
            myPayload.includeQRCode = true;
            string host = GetRequestHostName();
            if (!host.Contains("//localhost"))
            {
                callback.url = String.Format("{0}:/api/verifier/presentationCallback", host);
            }
            else
            {
            ...
            myAcceptedIssuers.Add(_configuration["AppSettings:IssuerAuthority"]);
            ...
            string myResponse = await _vcService.PostVPayloadAsync(token.access_token, myPayload);
            JObject requestConfig = JObject.Parse(myResponse);
        ...
        [HttpGet("/api/verifier/presentation-request2")]
        public async Task<ActionResult> PresentationRequest2()
        {
            try
            {
            ...
            try
            {
                ...
                if (presentationResponse["code"].ToString() == "presentation_verified")
                {
                    ...
                    var cacheData = new
                    {
                        ...
                        id = presentationResponse["issuers"][0]["claims"]["id"].ToString(),
                        imagefile = presentationResponse["issuers"][0]["claims"]["imageFile"].ToString(),
                    ...
                    _cache.Set(state, JsonConvert.SerializeObject(cacheData));
                    //For debugging
                    //_cache.Set("debug", JsonConvert.SerializeObject(content));
